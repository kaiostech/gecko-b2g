...
function verifyPreferredNetworkType(aExpectedType) {
  return getPreferredNetworkType().then(
    function resolve(aType) {
...
function setAndVerifyPreferredNetworkType(
  aType,
  aExpectedErrorMsg,
  aExpectGotException
) {
  ...
  try {
    return setPreferredNetworkType(aType).then(
      function resolve() {
...
startTestCommon(function() {
  ...
  for (let i = 0; i < TEST_DATA.length; i++) {
    ...
    promise = promise.then(() =>
      setAndVerifyPreferredNetworkType(
        data.preferredNetworkType,
