...
void doStatusReport(const nsCString& aInputStr) {
...
nsresult getStatus(nsACString& aDesc) {
...
static nsresult DumpReport(nsIFileOutputStream* aOStream,
                           const nsCString& aProcess, const nsCString& aName,
...
nsStatusReporterManager::DumpReports() {
  ...
  // The file is initialized as "incomplete-status-reports-pid-number.json" in
...
nsresult NS_RegisterStatusReporter(nsIStatusReporter* aReporter) {
  nsCOMPtr<nsIStatusReporterManager> mgr =
      do_GetService("@mozilla.org/status-reporter-manager;1");
  if (!mgr) {
    return NS_ERROR_FAILURE;
...
nsresult NS_UnregisterStatusReporter(nsIStatusReporter* aReporter) {
  nsCOMPtr<nsIStatusReporterManager> mgr =
      do_GetService("@mozilla.org/status-reporter-manager;1");
  if (!mgr) {
    return NS_ERROR_FAILURE;
