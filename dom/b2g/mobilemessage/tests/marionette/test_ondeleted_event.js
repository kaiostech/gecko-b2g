...
function verifyDeletedInfo(aDeletedInfo, aMsgIds, aThreadIds) {
  is(
    JSON.stringify(aDeletedInfo.deletedMessageIds),
    JSON.stringify(aMsgIds),
    "Check Msg Id."
  );
  is(
    JSON.stringify(aDeletedInfo.deletedThreadIds),
    JSON.stringify(aThreadIds),
    "Check Thread Id."
  );
}
...
function testDeletingMessagesInOneThreadOneByOne() {
  ...
  return Promise.resolve()
    ...
    .then(aResult => {
      verifyDeletedInfo(aResult.deletedInfo, [sentMessages[0].id], null);
    })
...
startTestCommon(function testCaseMain() {
  return testDeletingMessagesInOneThreadOneByOne().then(() =>
