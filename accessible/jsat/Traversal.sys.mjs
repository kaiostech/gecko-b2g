...
function BaseTraversalRule(aRoles, aMatchFunc, aPreFilter, aContainerRule) {
  ...
  if (aRoles.length) {
    if (aRoles.includes(Roles.LABEL) === false) {
      this._matchRoles.push(Roles.LABEL);
    }
    if (aRoles.includes(Roles.INTERNAL_FRAME) === false) {
...
var gSimpleMatchFunc = function gSimpleMatchFunc(aAccessible) {
  ...
  function isFlatSubtree(acc) {
    for (let child = acc.firstChild; child; child = child.nextSibling) {
      ...
      // listener.
      if ([Roles.TEXT_LEAF, Roles.STATICTEXT].includes(child.role) === true) {
        continue;
      }
