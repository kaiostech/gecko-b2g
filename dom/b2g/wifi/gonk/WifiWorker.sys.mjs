...
const LEVEL_EXCESSIVE = Ci.nsISupplicantDebugLevel.SUP_LOG_EXCESSIVE;
const LEVEL_MSGDUMP = Ci.nsISupplicantDebugLevel.SUP_LOG_MSGDUMP;
const LEVEL_DEBUG = Ci.nsISupplicantDebugLevel.SUP_LOG_DEBUG;
const LEVEL_INFO = Ci.nsISupplicantDebugLevel.SUP_LOG_INFO;
const LEVEL_WARNING = Ci.nsISupplicantDebugLevel.SUP_LOG_WARNING;
const LEVEL_ERROR = Ci.nsISupplicantDebugLevel.SUP_LOG_ERROR;
...
var WifiManager = (function () {
  ...
  function updateChannels(callback) {
    ...
    wifiCommand.getChannelsForBand(
      WifiScanSettings.bandMask,
      function (result) {
  ...
  function supplicantNetworkDisconnected(event) {
    ...
    if (manager.isWifiEnabled(manager.state)) {
      ...
      if (typeof configuredNetworks[networkKey] !== "undefined") {
        let networkEnabled =
          !configuredNetworks[networkKey].networkSelectionStatus;
        handleScanRequest(true, function () {});
        debug(
  ...
  netFromDOM = function (net, configured) {
    ...
    if (wep) {
      ...
      if (net.wep && net.wep != "*") {
        ...
        configured["wepKey" + net.keyIndex] = net["wepKey" + net.keyIndex] =
...
WifiWorker.prototype = {
  ...
  getCountryName() {
    for (let simId = 0; simId < WifiManager.numRil; simId++) {
      let countryName = lazy.PhoneNumberUtils.getCountryName(simId);
