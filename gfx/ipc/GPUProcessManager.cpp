...
bool GPUProcessManager::EnsureProtocolsReady() {
  return EnsureCompositorManagerChild() &&
#ifdef MOZ_WIDGET_GONK
         EnsureSharedBufferManagerChild() &&
#endif
         EnsureImageBridgeChild() && EnsureVRManager();
}
...
#ifdef MOZ_WIDGET_GONK
bool GPUProcessManager::EnsureSharedBufferManagerChild() {
...
bool GPUProcessManager::CreateContentSharedBufferManager(
  ...
  rv = PSharedBufferManager::CreateEndpoints(parentPid, aOtherProcess,
                                             &parentPipe, &childPipe);
  if (NS_FAILED(rv)) {
    gfxCriticalNote << "Could not create content compositor bridge: "
                    << hexa(int(rv));
