...
status_t GonkBufferQueueProducer::requestBuffer(int slot,
  ...
  if (mCore->mConnectedApi == GonkBufferQueueCore::NO_CONNECTED_API) {
    ALOGE("requestBuffer: BufferQueue has no connected producer");
    return NO_INIT;
  }
...
status_t GonkBufferQueueProducer::setMaxDequeuedBufferCount(
    int maxDequeuedBuffers) {
  ATRACE_CALL();
  ALOGV("setMaxDequeuedBufferCount: maxDequeuedBuffers = %d",
        maxDequeuedBuffers);
  ...
  {  // Autolock scope
    ...
    int bufferCount = mCore->getMinUndequeuedBufferCountLocked(false);
    bufferCount += maxDequeuedBuffers;

    if (bufferCount > GonkBufferQueueDefs::NUM_BUFFER_SLOTS) {
      ALOGE("setBufferCount: bufferCount %d too large (max %d)", bufferCount,
            GonkBufferQueueDefs::NUM_BUFFER_SLOTS);
      return BAD_VALUE;
    }
...
status_t GonkBufferQueueProducer::setAsyncMode(bool async) {
  return INVALID_OPERATION;
}

status_t GonkBufferQueueProducer::waitForFreeSlotThenRelock(
    const char* caller, bool async, int* found, status_t* returnFlags) const {
  bool tryAgain = true;
  while (tryAgain) {
    if (mCore->mIsAbandoned) {
    ...
    if (async && mCore->mOverrideMaxBufferCount) {
      ...
      if (mCore->mOverrideMaxBufferCount < maxBufferCount) {
        ALOGE("%s: async mode is invalid with buffer count override", caller);
        return BAD_VALUE;
      }
    }

    // Free up any buffers that are in slots beyond the max buffer count
    // for (int s = maxBufferCount; s < GonkBufferQueueDefs::NUM_BUFFER_SLOTS;
    // ++s) {
    ...
    *found = GonkBufferQueueCore::INVALID_BUFFER_SLOT;
    ...
    if (!mCore->mOverrideMaxBufferCount && dequeuedCount) {
      ALOGE(
          "%s: can't dequeue multiple buffers without setting the "
          "buffer count",
          caller);
      return INVALID_OPERATION;
    }
    ...
    if (mCore->mBufferHasBeenQueued) {
      ...
      if (newUndequeuedCount < minUndequeuedCount) {
        ALOGE(
            "%s: min undequeued buffer count (%d) exceeded "
            "(dequeued=%d undequeued=%d)",
            caller, minUndequeuedCount, dequeuedCount, newUndequeuedCount);
        return INVALID_OPERATION;
      }
    }
    ...
    if (tooManyBuffers) {
      ALOGV("%s: queue size is %zu, waiting", caller, mCore->mQueue.size());
    }
    ...
    tryAgain =
        (*found == GonkBufferQueueCore::INVALID_BUFFER_SLOT) || tooManyBuffers;
    if (tryAgain) {
...
status_t GonkBufferQueueProducer::dequeueBuffer(
    int* outSlot, sp<android::Fence>* outFence, uint32_t width, uint32_t height,
  ...
  ALOGV("dequeueBuffer: w=%u h=%u format=%#x, usage=%#" PRIx64, width, height,
        format, usage);
  ...
  *outSlot = GonkBufferQueueCore::INVALID_BUFFER_SLOT;
  ...
  {  // Autolock scope
    ...
    int found;
    status_t status =
        waitForFreeSlotThenRelock("dequeueBuffer", false, &found, &returnFlags);
    if (status != NO_ERROR) {
      return status;
    }
    ...
    *outSlot = found;
    ...
    if (CC_UNLIKELY(mSlots[found].mFence == NULL)) {
      ALOGE(
          "dequeueBuffer: about to return a NULL fence - "
...
status_t GonkBufferQueueProducer::detachBuffer(int slot) {
  ...
  if (slot < 0 || slot >= GonkBufferQueueDefs::NUM_BUFFER_SLOTS) {
    ALOGE("detachBuffer(P): slot index %d out of range [0, %d)", slot,
          GonkBufferQueueDefs::NUM_BUFFER_SLOTS);
    return BAD_VALUE;
  } else if (mSlots[slot].mBufferState != GonkBufferSlot::DEQUEUED) {
    ALOGE(
        "detachBuffer(P): slot %d is not owned by the producer "
        "(state = %d)",
        slot, mSlots[slot].mBufferState);
    return BAD_VALUE;
  } else if (!mSlots[slot].mRequestBufferCalled) {
    ALOGE("detachBuffer(P): buffer in slot %d has not been requested", slot);
    return BAD_VALUE;
  }
...
status_t GonkBufferQueueProducer::attachBuffer(
  ...
  status_t status =
      waitForFreeSlotThenRelock("attachBuffer(P)", false, &found, &returnFlags);
  if (status != NO_ERROR) {
    return status;
  }
  ...
  *outSlot = found;
  ...
  ALOGV("attachBuffer(P): returning slot %d flags=%#x", *outSlot, returnFlags);
...
status_t GonkBufferQueueProducer::queueBuffer(int slot,
  ...
  input.deflate(&timestamp, &isAutoTimestamp, &dataspace, &crop, &scalingMode,
                &transform, &fence, &stickyTransform, &getFrameTimestamps);

  if (fence == NULL) {
    ALOGE("queueBuffer: fence is NULL");
    // Temporary workaround for b/17946343: soldier-on instead of returning an
    // error. This prevents the client from dying, at the risk of visible
    // corruption due to hwcomposer reading the buffer before the producer is
    // done rendering it. Unless the buffer is the last frame of an animation,
    // the corruption will be transient.
  ...
  {  // Autolock scope
    ...
    if (async && mCore->mOverrideMaxBufferCount) {
      ...
      if (mCore->mOverrideMaxBufferCount < maxBufferCount) {
        ALOGE(
            "queueBuffer: async mode is invalid with "
    ...
    ALOGV("queueBuffer: slot=%d/%" PRIu64 " time=%" PRIu64
          ...
          slot, mCore->mFrameCounter + 1, timestamp, crop.left, crop.top,
          crop.right, crop.bottom, transform,
          GonkBufferItem::scalingModeName(scalingMode));
...
status_t GonkBufferQueueProducer::cancelBuffer(int slot,
  ...
  switch (api) {
    ...
    case NATIVE_WINDOW_API_CAMERA:
      ...
      if (listener != NULL &&
        ...
        if (status != NO_ERROR) {
          ALOGE("connect(P): linkToDeath failed: %s (%d)", strerror(-status),
                status);
        }
      }
...
status_t GonkBufferQueueProducer::disconnect(int api, DisconnectMode mode) {
  ATRACE_CALL();
    ...
    switch (api) {
      ...
      case NATIVE_WINDOW_API_CAMERA:
        if (mCore->mConnectedApi == api) {
          ...
          ALOGE(
              "disconnect(P): connected to another API "
              "(cur=%d req=%d)",
              mCore->mConnectedApi, api);
          status = BAD_VALUE;
        }
...
status_t GonkBufferQueueProducer::setSidebandStream(
    const sp<NativeHandle>& stream) {
  return INVALID_OPERATION;
}

void GonkBufferQueueProducer::allocateBuffers(uint32_t width, uint32_t height,
                                              PixelFormat format,
                                              uint64_t usage) {
...
status_t GonkBufferQueueProducer::setSharedBufferMode(bool sharedBufferMode) {
  return INVALID_OPERATION;
}

status_t GonkBufferQueueProducer::setAutoRefresh(bool autoRefresh) {
  return INVALID_OPERATION;
}

status_t GonkBufferQueueProducer::setDequeueTimeout(nsecs_t timeout) {
  return INVALID_OPERATION;
}

status_t GonkBufferQueueProducer::getLastQueuedBuffer(
    sp<GraphicBuffer>* outBuffer, sp<Fence>* outFence,
    float outTransformMatrix[16]) {
  return INVALID_OPERATION;
}

void GonkBufferQueueProducer::getFrameTimestamps(
    FrameEventHistoryDelta* outDelta) {}

void GonkBufferQueueProducer::binderDied(
