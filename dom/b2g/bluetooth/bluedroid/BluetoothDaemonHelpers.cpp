...
nsresult Convert(bool aIn, BluetoothScanMode& aOut) {
...
nsresult Convert(int32_t aIn, BluetoothTypeOfDevice& aOut) {
  static const BluetoothTypeOfDevice sTypeOfDevice[] = {
      [0x00] =
          static_cast<BluetoothTypeOfDevice>(0),  // invalid, required by gcc
      [0x01] = TYPE_OF_DEVICE_BREDR,
      [0x02] = TYPE_OF_DEVICE_BLE,
      [0x03] = TYPE_OF_DEVICE_DUAL};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(!aIn, int32_t, BluetoothTypeOfDevice) ||
...
nsresult Convert(int32_t aIn, BluetoothScanMode& aOut) {
  static const BluetoothScanMode sScanMode[] = {
      [0x00] = SCAN_MODE_NONE,
      [0x01] = SCAN_MODE_CONNECTABLE,
      [0x02] = SCAN_MODE_CONNECTABLE_DISCOVERABLE};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn < 0, int32_t, BluetoothScanMode) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(
...
nsresult Convert(uint8_t aIn, BluetoothA2dpAudioState& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sAudioState), uint8_t,
...
nsresult Convert(uint8_t aIn, BluetoothA2dpConnectionState& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sConnectionState),
                                  uint8_t, BluetoothA2dpConnectionState)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
...
nsresult Convert(uint8_t aIn, BluetoothAclState& aOut) {
  static const BluetoothAclState sAclState[] = {
      [0x00] = ACL_STATE_CONNECTED, [0x01] = ACL_STATE_DISCONNECTED};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sAclState), uint8_t,
                                  BluetoothAclState)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sAclState[aIn];
...
nsresult Convert(uint8_t aIn, BluetoothAvrcpEvent& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(!aIn, uint8_t, BluetoothAvrcpEvent) ||
      ...
      MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sAvrcpEvent), uint8_t,
                                  BluetoothAvrcpEvent)) {
...
nsresult Convert(uint8_t aIn, BluetoothAvrcpMediaAttribute& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(!aIn, uint8_t,
                                  BluetoothAvrcpMediaAttrbiute) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sAvrcpMediaAttribute),
                                  uint8_t, BluetoothAvrcpMediaAttribute)) {
...
nsresult Convert(uint8_t aIn, BluetoothAvrcpPlayerAttribute& aOut) {
...
nsresult Convert(uint8_t aIn, BluetoothAvrcpRemoteFeatureBits& aOut) {
...
nsresult Convert(uint8_t aIn, BluetoothBondState& aOut) {
  static const BluetoothBondState sBondState[] = {[0x00] = BOND_STATE_NONE,
                                                  [0x01] = BOND_STATE_BONDING,
                                                  [0x02] = BOND_STATE_BONDED};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sBondState), uint8_t,
                                  BluetoothBondState)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sBondState[aIn];
...
nsresult Convert(uint8_t aIn, BluetoothHandsfreeAudioState& aOut) {
...
nsresult Convert(uint8_t aIn, BluetoothHandsfreeCallHoldType& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sCallHoldType),
                                  uint8_t, BluetoothHandsfreeCallHoldType)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
...
nsresult Convert(uint8_t aIn, BluetoothHandsfreeConnectionState& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sConnectionState),
                                  uint8_t, BluetoothHandsfreeConnectionState)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sConnectionState[aIn];
  return NS_OK;
...
nsresult Convert(uint8_t aIn, BluetoothHandsfreeNRECState& aOut) {
  static const BluetoothHandsfreeNRECState sNRECState[] = {
      [0x00] = HFP_NREC_STOPPED, [0x01] = HFP_NREC_STARTED};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sNRECState), uint8_t,
...
nsresult Convert(uint8_t aIn, BluetoothHandsfreeVoiceRecognitionState& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sState), uint8_t,
...
nsresult Convert(uint8_t aIn, BluetoothHandsfreeVolumeType& aOut) {
  static const BluetoothHandsfreeVolumeType sVolumeType[] = {
      [0x00] = HFP_VOLUME_TYPE_SPEAKER, [0x01] = HFP_VOLUME_TYPE_MICROPHONE};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sVolumeType), uint8_t,
...
nsresult Convert(uint8_t aIn, BluetoothHandsfreeWbsConfig& aOut) {
  static const BluetoothHandsfreeWbsConfig sWbsConfig[] = {
      [0x00] = HFP_WBS_NONE, [0x01] = HFP_WBS_NO, [0x02] = HFP_WBS_YES};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sWbsConfig), uint8_t,
...
nsresult Convert(uint8_t aIn, BluetoothPropertyType& aOut) {
  static const BluetoothPropertyType sPropertyType[] = {
      [0x00] =
          static_cast<BluetoothPropertyType>(0),  // invalid, required by gcc
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(!aIn, uint8_t, BluetoothPropertyType) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sPropertyType),
                                  uint8_t, BluetoothPropertyType)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
...
nsresult Convert(BluetoothSocketType aIn, uint8_t& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn == BluetoothSocketType::EL2CAP,
                                  BluetoothSocketType, uint8_t) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sSocketType),
                                  BluetoothSocketType, uint8_t) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(!sSocketType[aIn], BluetoothSocketType,
                                  uint8_t)) {
...
nsresult Convert(uint8_t aIn, BluetoothSspVariant& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sSspVariant), uint8_t,
                                  BluetoothSspVariant)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
...
nsresult Convert(uint8_t aIn, BluetoothStatus& aOut) {
  static const BluetoothStatus sStatus[] = {
      [0x00] = STATUS_SUCCESS,      [0x01] = STATUS_FAIL,
      [0x02] = STATUS_NOT_READY,    [0x03] = STATUS_NOMEM,
      [0x04] = STATUS_BUSY,         [0x05] = STATUS_DONE,
      [0x06] = STATUS_UNSUPPORTED,  [0x07] = STATUS_PARM_INVALID,
      [0x08] = STATUS_UNHANDLED,    [0x09] = STATUS_AUTH_FAILURE,
      [0x0a] = STATUS_RMT_DEV_DOWN, [0x0b] = STATUS_AUTH_REJECTED};
...
nsresult Convert(int32_t aIn, BluetoothGattStatus& aOut) {
  /* Reference: $B2G/system/bt/stack/include/gatt_api.h */
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn < 0, int32_t, BluetoothGattStatus) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(
...
nsresult Convert(int32_t aIn, BluetoothGattDbType& aOut) {
  /* Reference: $B2G/system/bt/include/hardware/bt_common_types.h */
  static const BluetoothGattDbType sGattDbType[] = {
      [0x0000] = GATT_DB_TYPE_PRIMARY_SERVICE,
      [0x0001] = GATT_DB_TYPE_SECONDARY_SERVICE,
      [0x0002] = GATT_DB_TYPE_INCLUDED_SERVICE,
      [0x0003] = GATT_DB_TYPE_CHARACTERISTIC,
      [0x0004] = GATT_DB_TYPE_DESCRIPTOR};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn < 0, int32_t, BluetoothGattDbType) ||
      MOZ_HAL_IPC_CONVERT_WARN_IF(
          aIn >= static_cast<ssize_t>(MOZ_ARRAY_LENGTH(sGattDbType)), int32_t,
          BluetoothGattDbType)) {
    aOut = GATT_DB_TYPE_DESCRIPTOR;
  } else {
    aOut = sGattDbType[aIn];
  }
  return NS_OK;
}
...
nsresult Convert(uint8_t aIn, BluetoothHidProtocolMode& aOut) {
  static const BluetoothHidProtocolMode sMode[] = {
      [0x00] = HID_PROTOCOL_MODE_REPORT, [0x01] = HID_PROTOCOL_MODE_BOOT};
  if (aIn == 0xff) {
    /* This case is handled separately to not populate
...
nsresult Convert(uint8_t aIn, BluetoothHidConnectionState& aOut) {
...
nsresult Convert(uint8_t aIn, BluetoothHidStatus& aOut) {
...
nsresult Convert(int32_t aIn, BluetoothSdpType& aOut) {
  static const BluetoothSdpType sSdpType[] = {
      [0x00] = SDP_TYPE_RAW,       [0x01] = SDP_TYPE_MAP_MAS,
      [0x02] = SDP_TYPE_MAP_MNS,   [0x03] = SDP_TYPE_PBAP_PSE,
      [0x04] = SDP_TYPE_PBAP_PCE,  [0x05] = SDP_TYPE_OPP_SERVER,
      [0x06] = SDP_TYPE_SAP_SERVER};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(
          static_cast<size_t>(aIn) >= MOZ_ARRAY_LENGTH(sSdpType), int32_t,
          BluetoothSdpType)) {
...
nsresult Convert(nsresult aIn, BluetoothStatus& aOut) {
  if (NS_SUCCEEDED(aIn)) {
    aOut = STATUS_SUCCESS;
...
nsresult Convert(BluetoothAvrcpPlayerAttribute aIn, uint8_t& aOut) {
  static const uint8_t sValue[] = {[AVRCP_PLAYER_ATTRIBUTE_EQUALIZER] = 0x01,
...
nsresult Convert(BluetoothAvrcpRemoteFeatureBits aIn, unsigned long& aOut) {
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(
...
nsresult Convert(BluetoothAvrcpStatus aIn, uint8_t& aOut) {
  static const uint8_t sValue[] = {[AVRCP_STATUS_BAD_COMMAND] = 0x00,
                                   [AVRCP_STATUS_BAD_PARAMETER] = 0x01,
                                   [AVRCP_STATUS_NOT_FOUND] = 0x02,
                                   [AVRCP_STATUS_INTERNAL_ERROR] = 0x03,
                                   [AVRCP_STATUS_SUCCESS] = 0x04};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sValue),
                                  BluetoothAvrcpStatus, uint8_t)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sValue[aIn];
  return NS_OK;
...
nsresult Convert(BluetoothHandsfreeCallAddressType aIn, uint8_t& aOut) {
...
nsresult Convert(BluetoothHandsfreeCallDirection aIn, uint8_t& aOut) {
...
nsresult Convert(BluetoothHandsfreeCallState aIn, uint8_t& aOut) {
  static const uint8_t sCallState[] = {
      [HFP_CALL_STATE_ACTIVE] = 0x00,   [HFP_CALL_STATE_HELD] = 0x01,
      [HFP_CALL_STATE_DIALING] = 0x02,  [HFP_CALL_STATE_ALERTING] = 0x03,
      [HFP_CALL_STATE_INCOMING] = 0x04, [HFP_CALL_STATE_WAITING] = 0x05,
      [HFP_CALL_STATE_IDLE] = 0x06};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sCallState),
                                  BluetoothHandsfreeCallState, uint8_t)) {
    aOut = 0x00;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sCallState[aIn];
...
nsresult Convert(BluetoothHandsfreeCallMode aIn, uint8_t& aOut) {
  static const uint8_t sCallMode[] = {[HFP_CALL_MODE_VOICE] = 0x00,
                                      [HFP_CALL_MODE_DATA] = 0x01,
                                      [HFP_CALL_MODE_FAX] = 0x02};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sCallMode),
                                  BluetoothHandsfreeCallMode, uint8_t)) {
    aOut = 0x00;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sCallMode[aIn];
...
nsresult Convert(BluetoothHandsfreeNetworkState aIn, uint8_t& aOut) {
...
nsresult Convert(BluetoothHandsfreeWbsConfig aIn, uint8_t& aOut) {
  static const uint8_t sWbsConfig[] = {
      [HFP_WBS_NONE] = 0x00, [HFP_WBS_NO] = 0x01, [HFP_WBS_YES] = 0x02};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sWbsConfig),
                                  BluetoothHandsfreeWbsConfig, uint8_t)) {
    aOut = 0x00;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sWbsConfig[aIn];
...
nsresult Convert(BluetoothPropertyType aIn, uint8_t& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sPropertyType),
                                  BluetoothPropertyType, uint8_t)) {
    aOut = 0x00;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sPropertyType[aIn];
...
nsresult Convert(BluetoothScanMode aIn, int32_t& aOut) {
  static const int32_t sScanMode[] = {
      [SCAN_MODE_NONE] = 0x00,
      [SCAN_MODE_CONNECTABLE] = 0x01,
      [SCAN_MODE_CONNECTABLE_DISCOVERABLE] = 0x02};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sScanMode),
                                  BluetoothScanMode, int32_t)) {
    aOut = 0;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sScanMode[aIn];
...
nsresult Convert(BluetoothSetupServiceId aIn, uint8_t& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sServiceId),
...
nsresult Convert(BluetoothSspVariant aIn, uint8_t& aOut) {
  static const uint8_t sValue[] = {[SSP_VARIANT_PASSKEY_CONFIRMATION] = 0x00,
                                   [SSP_VARIANT_PASSKEY_ENTRY] = 0x01,
                                   [SSP_VARIANT_CONSENT] = 0x02,
                                   [SSP_VARIANT_PASSKEY_NOTIFICATION] = 0x03};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sValue),
                                  BluetoothSspVariant, uint8_t)) {
    aOut = 0;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sValue[aIn];
...
nsresult Convert(ControlPlayStatus aIn, uint8_t& aOut) {
  static const uint8_t sValue[] = {[PLAYSTATUS_STOPPED] = 0x00,
...
nsresult Convert(BluetoothGattAuthReq aIn, int32_t& aOut) {
  static const int32_t sGattAuthReq[] = {[GATT_AUTH_REQ_NONE] = 0x00,
                                         [GATT_AUTH_REQ_NO_MITM] = 0x01,
                                         [GATT_AUTH_REQ_MITM] = 0x02,
                                         [GATT_AUTH_REQ_SIGNED_NO_MITM] = 0x03,
                                         [GATT_AUTH_REQ_SIGNED_MITM] = 0x04};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sGattAuthReq),
                                  BluetoothGattAuthReq, int32_t)) {
...
nsresult Convert(BluetoothGattAuthReq aIn, uint8_t& aOut) {
  static const uint8_t sGattAuthReq[] = {[GATT_AUTH_REQ_NONE] = 0x00,
                                         [GATT_AUTH_REQ_NO_MITM] = 0x01,
                                         [GATT_AUTH_REQ_MITM] = 0x02,
                                         [GATT_AUTH_REQ_SIGNED_NO_MITM] = 0x03,
                                         [GATT_AUTH_REQ_SIGNED_MITM] = 0x04};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sGattAuthReq),
                                  BluetoothGattAuthReq, uint8_t)) {
    aOut = GATT_AUTH_REQ_NONE;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sGattAuthReq[aIn];
...
nsresult Convert(BluetoothGattWriteType aIn, int32_t& aOut) {
  static const int32_t sGattWriteType[] = {[GATT_WRITE_TYPE_NO_RESPONSE] = 0x01,
                                           [GATT_WRITE_TYPE_NORMAL] = 0x02,
                                           [GATT_WRITE_TYPE_PREPARE] = 0x03,
                                           [GATT_WRITE_TYPE_SIGNED] = 0x04};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sGattWriteType),
                                  BluetoothGattWriteType, int32_t)) {
...
nsresult Convert(BluetoothHidProtocolMode aIn, uint8_t& aOut) {
  ...
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sMode),
                                  BluetoothHidProtocolMode, uint8_t)) {
    aOut = 0x00;  // silences compiler warning
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sMode[aIn];
...
nsresult Convert(BluetoothHidReportType aIn, uint8_t& aOut) {
...
nsresult Convert(BluetoothSdpType aIn, int32_t& aOut) {
  static const int32_t sValue[] = {
      [SDP_TYPE_RAW] = 0x00,       [SDP_TYPE_MAP_MAS] = 0x01,
      [SDP_TYPE_MAP_MNS] = 0x02,   [SDP_TYPE_PBAP_PSE] = 0x03,
      [SDP_TYPE_PBAP_PCE] = 0x04,  [SDP_TYPE_OPP_SERVER] = 0x05,
      [SDP_TYPE_SAP_SERVER] = 0x06};
  if (MOZ_HAL_IPC_CONVERT_WARN_IF(aIn >= MOZ_ARRAY_LENGTH(sValue),
                                  BluetoothSdpType, int32_t)) {
    return NS_ERROR_ILLEGAL_VALUE;
  }
  aOut = sValue[aIn];
  return NS_OK;
...
inline nsresult Convert(const ConvertArray<Tin>& aIn, Tout& aOut) {
...
nsresult PackPDU(const BluetoothAddress& aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackArray<uint8_t>(aIn.mAddr, sizeof(aIn.mAddr)), aPDU);
}

nsresult PackPDU(const BluetoothAttributeHandle& aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothAttributeHandle, uint16_t>(aIn), aPDU);
}

nsresult PackPDU(const BluetoothAvrcpAttributeTextPairs& aIn,
...
nsresult PackPDU(const BluetoothAvrcpAttributeValuePairs& aIn,
                 DaemonSocketPDU& aPDU) {
  size_t i;
...
nsresult PackPDU(const BluetoothAvrcpElementAttribute& aIn,
                 DaemonSocketPDU& aPDU) {
  ...
  if (MOZ_HAL_IPC_PACK_WARN_IF(cstr.Length() == PR_UINT32_MAX,
...
nsresult PackPDU(BluetoothAvrcpEvent aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothAvrcpEvent, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(const BluetoothAvrcpEventParamPair& aIn,
                 DaemonSocketPDU& aPDU) {
  nsresult rv;
...
nsresult PackPDU(const BluetoothSdpRecord& aIn, DaemonSocketPDU& aPDU) {
  nsresult rv;
...
nsresult PackPDU(BluetoothAvrcpNotification aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothAvrcpNotification, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(BluetoothAvrcpPlayerAttribute aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothAvrcpPlayerAttribute, uint8_t>(aIn),
                 aPDU);
}
...
nsresult PackPDU(const BluetoothHandsfreeAtResponse& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeAtResponse, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeCallAddressType& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(
      PackConversion<BluetoothHandsfreeCallAddressType, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeCallDirection& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeCallDirection, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeCallMode& aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeCallMode, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeCallMptyType& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeCallMptyType, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeCallState& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeCallState, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeNetworkState& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeNetworkState, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeServiceType& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeServiceType, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeVolumeType& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeVolumeType, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothHandsfreeWbsConfig& aIn,
                 DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHandsfreeWbsConfig, uint8_t>(aIn),
                 aPDU);
}

nsresult PackPDU(const BluetoothProperty& aIn, DaemonSocketPDU& aPDU) {
  ...
  switch (aIn.mType) {
    case PROPERTY_BDNAME:
      rv = PackPDU(
          PackConversion<uint8_t, uint16_t>(aIn.mRemoteName.mLength),
          PackArray<uint8_t>(aIn.mRemoteName.mName, aIn.mRemoteName.mLength),
          aPDU);
      break;
    ...
    case PROPERTY_ADAPTER_SCAN_MODE:
      ...
      break;
    default:
      NS_ERROR("Invalid property for packing");
      return NS_ERROR_ILLEGAL_VALUE;
  }
  return rv;
}

nsresult PackPDU(const BluetoothPinCode& aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(aIn.mLength,
                 PackArray<uint8_t>(aIn.mPinCode, sizeof(aIn.mPinCode)), aPDU);
}

nsresult PackPDU(BluetoothPropertyType aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothPropertyType, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(BluetoothSspVariant aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothSspVariant, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(BluetoothScanMode aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothScanMode, int32_t>(aIn), aPDU);
}

nsresult PackPDU(BluetoothSetupServiceId aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothSetupServiceId, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(const BluetoothServiceName& aIn, DaemonSocketPDU& aPDU) {
  static const uint8_t sTerminator = '\0';

  nsresult rv = PackPDU(PackArray<uint8_t>(aIn.mName, sizeof(aIn.mName)), aPDU);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
nsresult PackPDU(BluetoothSocketType aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothSocketType, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(ControlPlayStatus aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<ControlPlayStatus, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(BluetoothTransport aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothTransport, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(const BluetoothUuid& aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackArray<uint8_t>(aIn.mUuid, sizeof(aIn.mUuid)), aPDU);
}
...
nsresult PackPDU(BluetoothGattWriteType aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothGattWriteType, int32_t>(aIn), aPDU);
}

nsresult PackPDU(const BluetoothHidInfoParam& aIn, DaemonSocketPDU& aPDU) {
  ...
  nsresult rv = PackPDU(aIn.mAttributeMask, aIn.mSubclass, aIn.mApplicationId,
                        aIn.mVendorId, aIn.mProductId, aIn.mVersion,
                        aIn.mCountryCode, aIn.mDescriptorLength, aPDU);
  if (NS_FAILED(rv)) {
    return rv;
  }
  return PackPDU(
...
nsresult PackPDU(const BluetoothHidReport& aIn, DaemonSocketPDU& aPDU) {
  uint8_t* reportData = const_cast<uint8_t*>(aIn.mReportData.Elements());
  ...
  return PackPDU(PackArray<uint8_t>(reportData, aIn.mReportData.Length()),
                 aPDU);
}

nsresult PackPDU(BluetoothHidProtocolMode aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHidProtocolMode, uint8_t>(aIn), aPDU);
}

nsresult PackPDU(BluetoothHidReportType aIn, DaemonSocketPDU& aPDU) {
  return PackPDU(PackConversion<BluetoothHidReportType, uint8_t>(aIn), aPDU);
}
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothA2dpAudioState& aOut) {
  return UnpackPDU(aPDU,
                   UnpackConversion<uint8_t, BluetoothA2dpAudioState>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothA2dpConnectionState& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothA2dpConnectionState>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothAclState& aOut) {
  return UnpackPDU(aPDU, UnpackConversion<uint8_t, BluetoothAclState>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothAttributeHandle& aOut) {
  return UnpackPDU(aPDU,
                   UnpackConversion<uint16_t, BluetoothAttributeHandle>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothAvrcpEvent& aOut) {
  return UnpackPDU(aPDU, UnpackConversion<uint8_t, BluetoothAvrcpEvent>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothAvrcpMediaAttribute& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothAvrcpMediaAttribute>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothAvrcpPlayerAttribute& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothAvrcpPlayerAttribute>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothAvrcpPlayerSettings& aOut) {
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU,
                   BluetoothAvrcpRemoteFeatureBits& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothAvrcpRemoteFeatureBits>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothBondState& aOut) {
  return UnpackPDU(aPDU, UnpackConversion<uint8_t, BluetoothBondState>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothTypeOfDevice& aOut) {
  return UnpackPDU(aPDU,
                   UnpackConversion<int32_t, BluetoothTypeOfDevice>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothHandsfreeAudioState& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothHandsfreeAudioState>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU,
                   BluetoothHandsfreeCallHoldType& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothHandsfreeCallHoldType>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU,
                   BluetoothHandsfreeConnectionState& aOut) {
  return UnpackPDU(
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothHandsfreeVolumeType& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothHandsfreeVolumeType>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothProperty& aOut) {
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothPropertyType& aOut) {
  return UnpackPDU(aPDU,
                   UnpackConversion<uint8_t, BluetoothPropertyType>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothRemoteInfo& aOut) {
  nsresult rv =
      UnpackPDU(aPDU, UnpackConversion<uint32_t, int>(aOut.mVerMajor));
  if (NS_FAILED(rv)) {
    return rv;
  }
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothScanMode& aOut) {
  return UnpackPDU(aPDU, UnpackConversion<int32_t, BluetoothScanMode>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothServiceRecord& aOut) {
  /* unpack UUID */
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothSspVariant& aOut) {
  return UnpackPDU(aPDU, UnpackConversion<uint8_t, BluetoothSspVariant>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothStatus& aOut) {
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothGattDbType& aOut) {
  return UnpackPDU(aPDU, UnpackConversion<int32_t, BluetoothGattDbType>(aOut));
}
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothGattReadParam& aOut) {
  /* unpack handle */
  nsresult rv = UnpackPDU(aPDU, aOut.mHandle);
  if (NS_FAILED(rv)) {
    return rv;
  }
  /* unpack status */
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothGattNotifyParam& aOut) {
  ...
  rv = UnpackPDU(aPDU, aOut.mHandle);
  if (NS_FAILED(rv)) {
    return rv;
  }
  /* unpack isNotify */
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU,
  ...
  int count = 0;
  nsresult rv = UnpackPDU(aPDU, count);
  for (int i = 0; i < count; ++i) {
    BluetoothGattDbElement dbElement;
    /* unpack id */
    nsresult rv = UnpackPDU(aPDU, dbElement.mId);
    if (NS_FAILED(rv)) {
      return rv;
    }
    /* unpack UUID */
    rv = UnpackPDU(aPDU, dbElement.mUuid);
    if (NS_FAILED(rv)) {
      return rv;
    }
    /* unpack type */
    rv = UnpackPDU(aPDU, dbElement.mType);
    if (NS_FAILED(rv)) {
      return rv;
    }
    /* unpack attribute handle */
    ...
    /* unpack start handle */
    rv = UnpackPDU(aPDU, dbElement.mStartHandle);
    if (NS_FAILED(rv)) {
      return rv;
    }
    /* unpack end handle */
    rv = UnpackPDU(aPDU, dbElement.mEndHandle);
    if (NS_FAILED(rv)) {
      return rv;
    }
    /* unpack properties */
    rv = UnpackPDU(aPDU, dbElement.mProperties);
    if (NS_FAILED(rv)) {
      return rv;
    }
    /* unpack permissions */
    rv = UnpackPDU(aPDU, dbElement.mPermissions);
    if (NS_FAILED(rv)) {
      return rv;
    }
    aOut.AppendElement(dbElement);
  }
  return rv;
}
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothHidInfoParam& aOut) {
  ...
  return UnpackPDU(aPDU, UnpackArray<uint8_t>(aOut.mDescriptorValue,
                                              aOut.mDescriptorLength));
}
...
nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothHidProtocolMode& aOut) {
  return UnpackPDU(aPDU,
                   UnpackConversion<uint8_t, BluetoothHidProtocolMode>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothHidConnectionState& aOut) {
  return UnpackPDU(
      aPDU, UnpackConversion<uint8_t, BluetoothHidConnectionState>(aOut));
}

nsresult UnpackPDU(DaemonSocketPDU& aPDU, BluetoothHidStatus& aOut) {
  return UnpackPDU(aPDU, UnpackConversion<uint8_t, BluetoothHidStatus>(aOut));
}
