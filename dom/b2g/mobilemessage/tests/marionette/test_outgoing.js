...
function doSingleRequest(aRequest, aReceiver, aBody, aNow) {
  ...
  promises.push(
    waitForManagerEvent("sending", isReceiverMatch.bind(null, aReceiver)).then(
      function(aEvent) {
        ...
        ok(
          Math.floor(sendingMessage.timestamp / 1000) >=
            Math.floor(aNow / 1000),
  ...
  promises.push(
    waitForManagerEvent("sent", isReceiverMatch.bind(null, aReceiver)).then(
      function(aEvent) {
  ...
  promises.push(
    aRequest.then(function(aResult) {
...
function doSendMessageAndCheckSuccess(receivers, body) {
  log(
    "Testing sending message(s) to receiver(s): " + JSON.stringify(receivers)
  );
  ...
  if (Array.isArray(receivers)) {
    ...
    return Promise.all(
      result.map(function(request, index) {
