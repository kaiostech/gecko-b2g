...
function checkMessage(aNeedle, aValidNumbers) {
  ...
  return getMessages(filter).then(function(messages) {
    ...
    for (let message of messages) {
      ...
      if (message.type == "sms") {
        number =
          message.delivery === "received" ? message.sender : message.receiver;
      } else {
...
startTestCommon(function testCaseMain() {
  return (
    Promise.resolve()
      ...
      .then(() =>
        sendTextSmsToEmulator(REMOTE_INTERNATIONAL_NUMBER + "2", TEXT_2)
      )
      // MMS, international
      .then(() =>
        sendMmsWithFailure(genFailingMms([REMOTE_INTERNATIONAL_NUMBER + "3"]))
      )
      // MMS, national
      .then(() =>
        sendMmsWithFailure(genFailingMms([REMOTE_NATIONAL_NUMBER + "3"]))
      )
      ...
      .then(() =>
        checkMessage(REMOTE_INTERNATIONAL_NUMBER + "1", [
          REMOTE_INTERNATIONAL_NUMBER + "1",
          REMOTE_NATIONAL_NUMBER + "1",
        ])
      )
      .then(() =>
        checkMessage(REMOTE_NATIONAL_NUMBER + "2", [
          REMOTE_NATIONAL_NUMBER + "2",
          REMOTE_INTERNATIONAL_NUMBER + "2",
        ])
      )
      .then(() =>
        checkMessage(REMOTE_INTERNATIONAL_NUMBER + "3", [
          REMOTE_INTERNATIONAL_NUMBER + "3",
          REMOTE_NATIONAL_NUMBER + "3",
        ])
      )
      .then(() =>
        checkMessage(REMOTE_NATIONAL_NUMBER + "3", [
          REMOTE_NATIONAL_NUMBER + "3",
          REMOTE_INTERNATIONAL_NUMBER + "3",
        ])
      )
      .then(() =>
        checkMessage(REMOTE_NATIONAL_NUMBER + "4", [
