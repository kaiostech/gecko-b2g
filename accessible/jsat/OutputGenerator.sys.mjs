...
var OutputGenerator = {
  ...
  genForContext: function genForContext(aContext) {
    ...
    if (this.outputOrder === OUTPUT_DESC_FIRST) {
      ...
      let doc,
        docIndex = contextStart.findIndex(
  ...
  _addMencloseNotations: function _addMencloseNotations(aOutput, aAccessible) {
    ...
    aOutput[this.outputOrder === OUTPUT_DESC_FIRST ? "push" : "unshift"].apply(
      ...
      notations.split(" ").map(notation => {
        this._getOutputName("notation-" + notation);
      })
    );
  },
  ...
  objectOutputFunctions: {
    ...
    mathmlenclosed: function mathmlenclosed(
      aAccessible,
      aRoleStr,
      aState,
      aFlags,
      ...
      let output = this.objectOutputFunctions.defaultFunc.apply(this, [
        aAccessible,
        aRoleStr,
        aState,
        aFlags,
...
export const UtteranceGenerator = {
  ...
  genForLiveRegion: function genForLiveRegion(
    aContext,
    aIsHide,
    aModifiedText
  ) {
  ...
  objectOutputFunctions: {
    ...
    cell: function cell(aAccessible, aRoleStr, aState, aFlags, aContext) {
      ...
      if (cell) {
        let addCellChanged = function addCellChanged(
          aUtterance,
          aChanged,
          aString,
          aIndex
  ...
  _getListUtterance: function _getListUtterance(
    aAccessible,
    aRoleStr,
    aFlags,
    aItemCount
...
export const BrailleGenerator = {
  ...
  objectOutputFunctions: {
    ...
    _useStateNotRole: function _useStateNotRole(
      aAccessible,
      aRoleStr,
      aState,
      aFlags
