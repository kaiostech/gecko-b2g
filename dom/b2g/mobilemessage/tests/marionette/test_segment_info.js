...
startTestCommon(function() {
  ...
  return (
    pushPrefEnv({ set: [["dom.sms.strict7BitEncoding", false]] })
      ...
      .then(() =>
        test(
          times("a", LEN_7BIT_WITH_8BIT_REF * 2 + 1),
          3,
          LEN_7BIT_WITH_8BIT_REF,
          LEN_7BIT_WITH_8BIT_REF - 1
        )
      )
      ...
      .then(() =>
        test(
          times("\u20ac", 1 + 2 * Math.floor(LEN_7BIT_WITH_8BIT_REF / 2)),
          3,
          LEN_7BIT_WITH_8BIT_REF,
          LEN_7BIT_WITH_8BIT_REF - 2
        )
      )
      // |2 * 76| single shift table chararacters + 1 locking shift table chararacter.
      .then(() =>
        test(
          "a" + times("\u20ac", 2 * Math.floor(LEN_7BIT_WITH_8BIT_REF / 2)),
          2,
          LEN_7BIT_WITH_8BIT_REF,
          1
        )
      ...
      .then(() =>
        test(
          times("\u20ac", 2 * Math.floor(LEN_7BIT_WITH_8BIT_REF / 2)) + "a",
          2,
          LEN_7BIT_WITH_8BIT_REF,
          0
        )
      ...
      .then(() =>
        test(
          ...
          LEN_UCS2_WITH_8BIT_REF - 4
        )
      )
      // |LEN_UCS2_WITH_8BIT_REF * 2| ucs2 chararacters.
      .then(() =>
        test(
          times("\u6afb", LEN_UCS2_WITH_8BIT_REF * 2),
          2,
          LEN_UCS2_WITH_8BIT_REF,
          0
        )
      ...
      .then(() =>
        test(
          times("\u6afb", LEN_UCS2_WITH_8BIT_REF * 2 + 1),
          3,
          LEN_UCS2_WITH_8BIT_REF,
          LEN_UCS2_WITH_8BIT_REF - 1
        )
      )
