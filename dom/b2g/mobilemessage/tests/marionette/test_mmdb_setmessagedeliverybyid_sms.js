...
function verify(
  ...
  log(
    "  Verify(" + aMessageId + ", " + aDelivery + ", " + aDeliveryStatus + ")"
  );
    ...
    function(aError, aTransaction, aMessageStore) {
      ...
      aTransaction.oncomplete = function() {
        ...
        if (aDelivery == DELIVERY_SENT) {
          ok(
            messageRecord.sentTimestamp >= messageRecord.timestamp,
            "messageRecord.sentTimestamp"
          );
        }
        if (aDeliveryStatus == DELIVERY_STATUS_SUCCESS) {
          ok(
            messageRecord.deliveryTimestamp >= messageRecord.timestamp,
...
function test(aTitle, aMmdb, aDeliveryStatusRequested, aParamArray) {
  ...
  return saveSendingMessage(aMmdb, message).then(function(aValues) {
    ...
    while (aParamArray.length) {
      ...
      promise = promise
        .then(() => {
          ...
          return setMessageDeliveryByMessageId(
            aMmdb,
            domMessage.id,
            null,
            params[0],
...
startTestBase(function testCaseMain() {
  return initMobileMessageDB(newMobileMessageDB(), DBNAME, 0)
    .then(aMmdb =>
      test(
        "Simulate send failed without delivery report requisition",
        aMmdb,
        false,
        [
          [
    ...
    .then(aMmdb =>
      test(
        "Simulate send failed with delivery report requisition",
        aMmdb,
        true,
        [
          [
    ...
    .then(aMmdb =>
      test("Simulate sent without delivery report requisition", aMmdb, false, [
        [DELIVERY_SENT, null, DELIVERY_SENT, DELIVERY_STATUS_NOT_APPLICABLE],
      ])
    )
    .then(aMmdb =>
      test("Simulate sent with delivery report success", aMmdb, true, [
        [DELIVERY_SENT, null, DELIVERY_SENT, DELIVERY_STATUS_PENDING],
        [null, DELIVERY_STATUS_SUCCESS, DELIVERY_SENT, DELIVERY_STATUS_SUCCESS],
      ])
    )
    .then(aMmdb =>
      test("Simulate sent with delivery report error", aMmdb, true, [
        [DELIVERY_SENT, null, DELIVERY_SENT, DELIVERY_STATUS_PENDING],
        [null, DELIVERY_ERROR, DELIVERY_SENT, DELIVERY_ERROR],
      ])
    )
