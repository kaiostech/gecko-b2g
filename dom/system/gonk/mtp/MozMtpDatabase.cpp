...
#  define MutexAutoLock              \
    MTP_LOG("About to enter mutex"); \
    DebugMutexAutoLock

#endif

static const char* ObjectPropertyAsStr(MtpObjectProperty aProperty) {
  switch (aProperty) {
    case MTP_PROPERTY_STORAGE_ID:
      return "MTP_PROPERTY_STORAGE_ID";
    case MTP_PROPERTY_OBJECT_FORMAT:
      return "MTP_PROPERTY_OBJECT_FORMAT";
    case MTP_PROPERTY_PROTECTION_STATUS:
      return "MTP_PROPERTY_PROTECTION_STATUS";
    case MTP_PROPERTY_OBJECT_SIZE:
      return "MTP_PROPERTY_OBJECT_SIZE";
    case MTP_PROPERTY_OBJECT_FILE_NAME:
      return "MTP_PROPERTY_OBJECT_FILE_NAME";
    case MTP_PROPERTY_DATE_CREATED:
      return "MTP_PROPERTY_DATE_CREATED";
    case MTP_PROPERTY_DATE_MODIFIED:
      return "MTP_PROPERTY_DATE_MODIFIED";
    case MTP_PROPERTY_PARENT_OBJECT:
      return "MTP_PROPERTY_PARENT_OBJECT";
    case MTP_PROPERTY_PERSISTENT_UID:
      return "MTP_PROPERTY_PERSISTENT_UID";
    case MTP_PROPERTY_NAME:
      return "MTP_PROPERTY_NAME";
    case MTP_PROPERTY_DATE_ADDED:
      return "MTP_PROPERTY_DATE_ADDED";
    case MTP_PROPERTY_WIDTH:
      return "MTP_PROPERTY_WIDTH";
    case MTP_PROPERTY_HEIGHT:
      return "MTP_PROPERTY_HEIGHT";
    case MTP_PROPERTY_IMAGE_BIT_DEPTH:
      return "MTP_PROPERTY_IMAGE_BIT_DEPTH";
    case MTP_PROPERTY_DISPLAY_NAME:
      return "MTP_PROPERTY_DISPLAY_NAME";
  }
  return "MTP_PROPERTY_???";
}

static char* FormatDate(time_t aTime, char* aDateStr, size_t aDateStrSize) {
  struct tm tm;
...
void MozMtpDatabase::AddEntry(DbEntry* entry) {
  MutexAutoLock lock(mMutex);
  ...
  MTP_DBG("Handle: 0x%08x Parent: 0x%08x Path:'%s'", entry->mHandle,
          entry->mParent, entry->mPath.get());
}
...
MtpObjectHandle MozMtpDatabase::FindEntryByPath(const nsACString& aPath) {
  MutexAutoLock lock(mMutex);
...
already_AddRefed<MozMtpDatabase::DbEntry> MozMtpDatabase::GetEntry(
...
void MozMtpDatabase::RemoveEntry(MtpObjectHandle aHandle) {
  MutexAutoLock lock(mMutex);
  ...
  // Since the index for a directory will always be less than the index of any
  // of its children, we can remove the entire subtree in one pass.
  ProtectedDbArray::size_type numEntries = mDb.Length();
  ProtectedDbArray::index_type entryIndex;
...
void MozMtpDatabase::UpdateEntry(MtpObjectHandle aHandle,
                                 DeviceStorageFile* aFile) {
  MutexAutoLock lock(mMutex);
...
#if USE_DEBUG
  char dateStr[20];
  MTP_DBG("UpdateEntry (0x%08x file %s) modified (%ld) %s", entry->mHandle,
...
class MtpWatcherNotifyRunnable final : public Runnable {
 public:
  explicit MtpWatcherNotifyRunnable(nsACString& aStorageName, nsACString& aPath,
                                    const char* aEventType)
      : Runnable("MtpWatcherNotify"),
  ...
  NS_IMETHOD Run() {
    ...
    RefPtr<DeviceStorageFile> dsf(new DeviceStorageFile(
        NS_LITERAL_STRING_FROM_CSTRING(DEVICESTORAGE_SDCARD), storageName,
        path));
    NS_ConvertUTF8toUTF16 eventType(mEventType);
    nsCOMPtr<nsIObserverService> obs = mozilla::services::GetObserverService();

    MTP_DBG("Sending mtp-watcher-notify %s %s %s", mEventType.get(),
            mStorageName.get(), mPath.get());
...
void MozMtpDatabase::MtpWatcherNotify(DbEntry* aEntry, const char* aEventType) {
  ...
  nsAutoCString relPath(
      Substring(aEntry->mPath, storageEntry->mStoragePath.Length() + 1));

  RefPtr<MtpWatcherNotifyRunnable> r = new MtpWatcherNotifyRunnable(
      storageEntry->mStorageName, relPath, aEventType);
  DebugOnly<nsresult> rv = NS_DispatchToMainThread(r);
  MOZ_ASSERT(NS_SUCCEEDED(rv));
}

// Called to tell the MTP server about new or deleted files,
void MozMtpDatabase::MtpWatcherUpdate(RefCountedMtpServer* aMtpServer,
                                      DeviceStorageFile* aFile,
  ...
  if (aEventType.EqualsLiteral("modified")) {
    ...
    if (entryHandle != 0) {
      ...
      MTP_LOG("About to update handle 0x%08x file %s", entryHandle,
              filePath.get());
  ...
  if (aEventType.EqualsLiteral("deleted")) {
    ...
    MTP_LOG("About to call sendObjectRemoved Handle 0x%08x file %s",
            entryHandle, filePath.get());
    RemoveEntryAndNotify(entryHandle, aMtpServer);
    return;
  }
}

nsCString MozMtpDatabase::BaseName(const nsCString& path) {
...
static nsCString GetPathWithoutFileName(const nsCString& aFullPath) {
  nsCString path;
...
void MozMtpDatabase::CreateEntryForFileAndNotify(
    const nsACString& aPath, DeviceStorageFile* aFile,
    RefCountedMtpServer* aMtpServer) {
  ...
  do {
    ...
    MTP_LOG("About to call sendObjectAdded Handle 0x%08x file %s",
...
void MozMtpDatabase::AddDirectory(MtpStorageID aStorageID, const char* aPath,
...
MozMtpDatabase::StorageArray::index_type MozMtpDatabase::FindStorage(
...
MtpStorageID MozMtpDatabase::FindStorageIDFor(const nsACString& aPath,
  ...
  for (storageIndex = 0; storageIndex < numStorages; storageIndex++) {
    RefPtr<StorageEntry> storage = mStorage[storageIndex];
    if (StringHead(aPath, storage->mStoragePath.Length())
            .Equals(storage->mStoragePath)) {
...
void MozMtpDatabase::AddStorage(MtpStorageID aStorageID, const char* aPath,
                                const char* aName) {
  // This is called on the IOThread from MozMtpStorage::StorageAvailable
  MOZ_ASSERT(MessageLoop::current() == XRE_GetIOMessageLoop());

  MTP_DBG("StorageID: 0x%08x aPath: '%s' aName: '%s'", aStorageID, aPath,
          aName);
...
void MozMtpDatabase::RemoveStorage(MtpStorageID aStorageID) {
  MutexAutoLock lock(mMutex);
...
// virtual
MtpObjectHandle MozMtpDatabase::beginSendObject(const char* aPath,
  ...
  if (aModified != 0) {
    ...
    // 1438482120.
    //
    // What we want is what mktime would pass us with the same date. Using
    // python (because its simple) with the current timezone set to be
    // America/Vancouver:
    //
    // >>> import time
    ...
    // time.struct_time(tm_year=2015, tm_mon=8, tm_mday=2, tm_hour=2, tm_min=22,
    // tm_sec=0, tm_wday=6, tm_yday=214, tm_isdst=1)
    //
...
// virtual
void MozMtpDatabase::endSendObject(MtpObjectHandle aHandle, bool aSucceeded) {
  MTP_LOG("Handle: 0x%08x '", aHandle);
...
// virtual
void MozMtpDatabase::rescanFile(const char* path, MtpObjectHandle aHandle,
...
// virtual
MtpObjectHandleList* MozMtpDatabase::getObjectList(MtpStorageID aStorageID,
                                                   MtpObjectFormat aFormat,
                                                   MtpObjectHandle aParent) {
  MTP_LOG("StorageID: 0x%08x Format: 0x%04x Parent: 0x%08x", aStorageID,
          aFormat, aParent);
...
// virtual
int MozMtpDatabase::getNumObjects(MtpStorageID aStorageID,
                                  MtpObjectFormat aFormat,
                                  MtpObjectHandle aParent) {
  MTP_LOG("");
...
MtpObjectFormatList* MozMtpDatabase::getSupportedPlaybackFormats() {
  static const uint16_t init_data[] = {
      MTP_FORMAT_UNDEFINED,     MTP_FORMAT_ASSOCIATION,   MTP_FORMAT_TEXT,
      MTP_FORMAT_HTML,          MTP_FORMAT_WAV,           MTP_FORMAT_MP3,
      MTP_FORMAT_MPEG,          MTP_FORMAT_EXIF_JPEG,     MTP_FORMAT_TIFF_EP,
      MTP_FORMAT_BMP,           MTP_FORMAT_GIF,           MTP_FORMAT_PNG,
      MTP_FORMAT_TIFF,          MTP_FORMAT_WMA,           MTP_FORMAT_OGG,
      MTP_FORMAT_AAC,           MTP_FORMAT_MP4_CONTAINER, MTP_FORMAT_MP2,
      MTP_FORMAT_3GP_CONTAINER, MTP_FORMAT_FLAC};

  MtpObjectFormatList* list = new MtpObjectFormatList();
  for (unsigned int i = 0; i < MOZ_ARRAY_LENGTH(init_data); i++) {
    list->push_back(init_data[i]);
  }
...
// virtual
MtpObjectFormatList* MozMtpDatabase::getSupportedCaptureFormats() {
...
// virtual
MtpObjectPropertyList* MozMtpDatabase::getSupportedObjectProperties(
    MtpObjectFormat aFormat) {
  MTP_LOG("");
  MtpObjectPropertyList* list = new MtpObjectPropertyList();
  for (unsigned int i = 0; i < MOZ_ARRAY_LENGTH(sSupportedObjectProperties);
       i++) {
    list->push_back(sSupportedObjectProperties[i]);
  }
...
// virtual
MtpDevicePropertyList* MozMtpDatabase::getSupportedDeviceProperties() {
  MTP_LOG("");
...
// virtual
MtpResponseCode MozMtpDatabase::getObjectPropertyValue(
    MtpObjectHandle aHandle, MtpObjectProperty aProperty,
  ...
  MTP_LOG("Handle: 0x%08x '%s' Property: %s 0x%08x", aHandle,
          entry->mDisplayName.get(), ObjectPropertyAsStr(aProperty), aProperty);
  ...
  switch (aProperty) {
    ...
    case MTP_PROPERTY_OBJECT_SIZE:
      ...
      break;
    case MTP_PROPERTY_DISPLAY_NAME:
      aPacket.putString(entry->mDisplayName.get());
      break;
    case MTP_PROPERTY_PERSISTENT_UID:
...
static int GetTypeOfObjectProp(MtpObjectProperty aProperty) {
...
// updateSubObjectPath is used to update the mPath of the sub folders or files
// when a folder was renamed
MtpResponseCode MozMtpDatabase::updateSubObjectPath(MtpObjectHandle aHandle,
  ...
  for (entryIdx = aHandle + 1; entryIdx < numEntries; entryIdx++) {
    ...
    // find the sub entry
    for (tmpIdx = entryIdx, tmpEntry = mDb[entryIdx];
         (MTP_PARENT_ROOT != tmpIdx) && mDb[tmpIdx];
         tmpIdx = mDb[tmpIdx]->mParent) {
      if (aHandle == mDb[tmpIdx]->mParent) {
        // update the mPath of each sub entry
        MtpWatcherNotify(tmpEntry, "deleted");
        tmpEntry->mPath.Replace(0, parentEntry->mPath.Length(),
                                newFileFullPath.get(),
                                newFileFullPath.Length());
...
// virtual
MtpResponseCode MozMtpDatabase::setObjectPropertyValue(
    MtpObjectHandle aHandle, MtpObjectProperty aProperty,
    MtpDataPacket& aPacket) {
  ...
  if (PR_Rename(entry->mPath.get(), newFileFullPath.get()) != PR_SUCCESS) {
    MTP_ERR("Failed to rename '%s' to '%s'", entry->mPath.get(),
            newFileFullPath.get());
    return MTP_RESPONSE_GENERAL_ERROR;
  }
  ...
  if (MTP_FORMAT_ASSOCIATION == entry->mObjectFormat) {
    MTP_DBG(
        "folder rename operation: sub folders and files path update needed "
        "'%s'",
        newFileFullPath.get());
...
// virtual
MtpResponseCode MozMtpDatabase::getDevicePropertyValue(
    MtpDeviceProperty aProperty, MtpDataPacket& aPacket) {
  MTP_LOG("(GENERAL ERROR)");
  return MTP_RESPONSE_GENERAL_ERROR;
}

// virtual
MtpResponseCode MozMtpDatabase::setDevicePropertyValue(
    MtpDeviceProperty aProperty, MtpDataPacket& aPacket) {
...
// virtual
MtpResponseCode MozMtpDatabase::resetDeviceProperty(
    MtpDeviceProperty aProperty) {
  MTP_LOG("(NOT SUPPORTED)");
  return MTP_RESPONSE_OPERATION_NOT_SUPPORTED;
}

void MozMtpDatabase::QueryEntries(MozMtpDatabase::MatchType aMatchType,
                                  uint32_t aMatchField1, uint32_t aMatchField2,
  ...
  switch (aMatchType) {
    ...
    case MatchParentFormat:
      for (entryIdx = 0; entryIdx < numEntries; entryIdx++) {
        entry = mDb[entryIdx];
        if (entry && entry->mParent == aMatchField1 &&
            entry->mObjectFormat == aMatchField2) {
          result.AppendElement(entry);
        }
      }
      break;
...
// virtual
MtpResponseCode MozMtpDatabase::getObjectPropertyList(
    MtpObjectHandle aHandle, uint32_t aFormat, uint32_t aProperty,
    int aGroupCode, int aDepth, MtpDataPacket& aPacket) {
  MTP_LOG(
      "Handle: 0x%08x Format: 0x%08x aProperty: 0x%08x aGroupCode: %d aDepth "
      "%d",
  ...
  for (entryIdx = 0; entryIdx < numEntries; entryIdx++) {
    ...
    for (size_t propertyIdx = 0; propertyIdx < numObjectProperties;
         propertyIdx++) {
      ...
      switch (prop) {
        ...
        case MTP_PROPERTY_DATE_CREATED: {
          aPacket.putUInt16(MTP_TYPE_STR);
          aPacket.putString(
              FormatDate(entry->mDateCreated, dateStr, sizeof(dateStr)));
        ...
        case MTP_PROPERTY_DATE_ADDED: {
          aPacket.putUInt16(MTP_TYPE_STR);
          aPacket.putString(
              FormatDate(entry->mDateAdded, dateStr, sizeof(dateStr)));
...
// virtual
MtpResponseCode MozMtpDatabase::getObjectInfo(MtpObjectHandle aHandle,
                                              MtpObjectInfo& aInfo) {
  RefPtr<DbEntry> entry = GetEntry(aHandle);
  if (!entry) {
  ...
  MTP_LOG("Handle: 0x%08x Display:'%s' Object:'%s'", aHandle,
          entry->mDisplayName.get(), entry->mObjectName.get());
...
// virtual
void* MozMtpDatabase::getThumbnail(MtpObjectHandle aHandle,
                                   size_t& aOutThumbSize) {
...
// virtual
MtpResponseCode MozMtpDatabase::getObjectFilePath(MtpObjectHandle aHandle,
  ...
  aOutFilePath.set(entry->mPath.get());
...
// virtual
MtpResponseCode MozMtpDatabase::beginDeleteObject(MtpObjectHandle aHandle) {
  RefPtr<DbEntry> entry = GetEntry(aHandle);
  if (!entry) {
    MTP_ERR("Invalid Handle: 0x%08x", aHandle);
    return MTP_RESPONSE_INVALID_OBJECT_HANDLE;
...
// virtual
MtpObjectHandleList* MozMtpDatabase::getObjectReferences(
    MtpObjectHandle aHandle) {
  MTP_LOG("Handle: 0x%08x (returning nullptr)", aHandle);
  return nullptr;
}
...
// virtual
MtpResponseCode MozMtpDatabase::setObjectReferences(
    MtpObjectHandle aHandle, MtpObjectHandleList* aReferences) {
  MTP_LOG("Handle: 0x%08x (NOT SUPPORTED)", aHandle);
  return MTP_RESPONSE_OPERATION_NOT_SUPPORTED;
}

// virtual
MtpProperty* MozMtpDatabase::getObjectPropertyDesc(MtpObjectProperty aProperty,
                                                   MtpObjectFormat aFormat) {
...
// virtual
MtpProperty* MozMtpDatabase::getDevicePropertyDesc(
    MtpDeviceProperty aProperty) {
...
// virtual
void MozMtpDatabase::endMoveObject(MtpObjectHandle oldParent,
                                   MtpObjectHandle newParent,
