...
export const Utils = {
  ...
  _embedderScreenReader: {},

  set Embedder(value) {
    this._embedderScreenReader = value;
  },
  ...
  get AccRetrieval() {
    if (!this._AccRetrieval) {
      this._AccRetrieval = Cc["@mozilla.org/accessibilityService;1"].getService(
  ...
  get AllMessageManagers() {
    ...
    if (document) {
      ...
      let remoteframes = document.querySelectorAll("browser");
  ...
  getBounds: function getBounds(aAccessible, aPreserveContentScale) {
    ...
    let scale = aPreserveContentScale
      ? 1
  ...
  dispatchChromeEvent: function dispatchChromeEvent(aType, aDetails) {
    ...
    this._embedderScreenReader.showScreenReader(details);
  },
...
State.prototype = {
  ...
  toString: function State_toString() {
    let stateStrings = Utils.AccRetrieval.getStringStates(
      this.base,
      this.extended
    );
...
export const Logger = {
  ...
  accessibleToString: function accessibleToString(aAccessible) {
    ...
    try {
      return (
        "[ " +
        Utils.AccRetrieval.getStringRole(aAccessible.role) +
        " | " +
        aAccessible.name +
        " ]"
      );
...
PivotContext.prototype = {
  ...
  get parentsDiff() {
    if (
      this._getParent(this.accessible) === this._getParent(this._oldAccessible)
    ) {
      return [];
    }
    if (!this._parentsDiff) {
      this._parentsDiff = this._ignoreAncestry
        ? []
        : this.currentAncestry.filter((currentAncestor, i) => {
            return (
              currentAncestor !== this.oldAncestry[i] ||
    ...
    this.newAncestry
      ...
      .forEach(aAccessible => {
        ...
        if (hint) {
          ...
          hints.push({
            string:
              Utils.AccRetrieval.getStringRole(aAccessible.role).replace(
                /\s/g,
                ""
              ) + "-hint",
          });
...
export const SettingCache = function SettingCache(
  ...
  req.addEventListener("success", () => {
    this.value =
      req.result[aName] === undefined
        ? aOptions.defaultValue
        : req.result[aName];
    if (aOptions.callbackNow) {
      runCallback();
    }
  ...
  settings.addObserver(aName, evt => {
