...
class MethodDefiner(PropertyDefiner):
    ...
    def __init__(self, descriptor, name, crossOriginOnly, static, unforgeable=False):
        ...
        def hasAsyncIterator(methods, regular):
            return any("@@asyncIterator" in m.aliases for m in methods) or any(
                "@@asyncIterator" == r["name"] for r in regular
            )
        ...
        if not static and not unforgeable and descriptor.supportsIndexedProperties():
            if hasIterator(methods, self.regular) or hasAsyncIterator(
                methods, self.regular
            ):
                raise TypeError(
