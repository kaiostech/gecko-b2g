 ...
 public:
  ...
  bool WorkerRun(JSContext* aCx, WorkerPrivate* aWorkerPrivate) override {
    ...
    RefPtr<Notification> notification = Notification::ConstructFromFields(
        ...
        mIcon, mImage, mData, mRequireInteraction, mActions, mSilent, mScope,
        result);
    if (NS_WARN_IF(result.Failed())) {
      return false;
    }
...
nsresult ServiceWorkerPrivate::SendNotificationEvent(
  ...
  if (mInner) {
    return mInner->SendNotificationEvent(
        ...
        aRequireInteraction, aActions, aUserAction, aSilent, aBehavior, aScope,
        gDOMDisableOpenClickDelay);
  }
  ...
  RefPtr<WorkerRunnable> r = new SendNotificationEventRunnable(
      ...
      aIcon, aImage, aData, aRequireInteraction, aActions, aUserAction, aSilent,
      aBehavior, aScope);
  if (NS_WARN_IF(!r->Dispatch())) {
    return NS_ERROR_FAILURE;
  }
