  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      manager.addEventListener("sending", function onevent(aEvent) {
        ...
        ok(
          Math.floor(message.timestamp / 1000) >= Math.floor(now / 1000),
 ...
 */
function turnOnVoiceDeleteMessagesAndVerify(
  aMessageIdsToDelete,
    ...
    new Promise(function(resolve, reject) {
      try {
        ...
        let tryResolve = function() {
          ...
          if (
            sentEventCount === aExpectedSentMessages.length &&
...
startTestCommon(function testCaseMain() {
  return (
    pushPrefEnv({ set: [["dom.sms.requestStatusReport", true]] })
      ...
      .then(aMessageIds =>
        turnOnVoiceDeleteMessagesAndVerify(
          [aMessageIds[0]],
          MESSAGES.slice().splice(1, MESSAGES.length - 1),
          1
        )
      )
  );
