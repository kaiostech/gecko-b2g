...
void GonkBufferQueueCore::dump(String8& result, const char* prefix) const {
  ...
  while (current != mQueue.end()) {
    fifo.appendFormat(
        ...
        current->mSlot, current->mGraphicBuffer.get(), current->mCrop.left,
        current->mCrop.top, current->mCrop.right, current->mCrop.bottom,
        current->mTransform, current->mTimestamp,
        GonkBufferItem::scalingModeName(current->mScalingMode));
    ++current;
  }

  result.appendFormat(
      "%s-GonkBufferQueue mMaxAcquiredBufferCount=%d, "
  ...
  for (int s = 0; s < maxBufferCount; ++s) {
    ...
    result.appendFormat(
        "%s%s[%02d:%p] state=%-8s", prefix,
        (slot.mBufferState == GonkBufferSlot::ACQUIRED) ? ">" : " ", s,
        buffer.get(), GonkBufferSlot::bufferStateName(slot.mBufferState));
...
status_t GonkBufferQueueCore::setDefaultMaxBufferCountLocked(int count) {
  ...
  if (count < minBufferCount || count > GonkBufferQueueDefs::NUM_BUFFER_SLOTS) {
    ALOGV(
        "setDefaultMaxBufferCount: invalid count %d, should be in "
        "[%d, %d]",
...
bool GonkBufferQueueCore::stillTracking(const GonkBufferItem* item) const {
  ...
  ALOGV("stillTracking: item { slot=%d/%" PRIu64
        " buffer=%p } "
