...
function testSetCallForwardingOption(aOptions) {
  ...
  promises.push(
    setCallForwardingOption(aOptions).then(null, aError => {
  ...
  return getCallForwardingOption(aReason).then(
    function resolve(aResults) {
      ...
      for (let i = 0; i < aResults.length; i++) {
        ...
        let expectedActive =
          aExpectedResult.action ===
            MozMobileConnection.CALL_FORWARD_ACTION_ENABLE ||
          aExpectedResult.action ===
...
startTestCommon(function() {
  ...
  for (let i = 0; i < TEST_DATA.length; i++) {
    ...
    promise = promise
      .then(() => testSetCallForwardingOption(data))
