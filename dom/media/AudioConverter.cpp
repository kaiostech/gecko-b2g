...
{
  // Front-end passes aAudioBalance with value from 0 to 100. If aAudioBalance
  // is
  ...
  if (aAudioBalance < DEFAULT_BALANCE) {
    // modify right channel
    offset = 1;
    factor = (float)aAudioBalance / DEFAULT_BALANCE;
  } else {
    // modify left channel
    offset = 0;
    // The original formula is
    // (DEFAULT_AUDIO_BALANCE - (aAudioBalance -
    // DEFAULT_AUDIO_BALANCE))/DEFAULT_AUDIO_BALANCE;
    // DEFAULT_AUDIO_BALANCE is 50.
    factor = (float)(100 - aAudioBalance) / DEFAULT_BALANCE;
  }
...
size_t AudioConverter::ProcessInternal(void* aOut, const void* aIn,
  ...
  if (mOut.AudioBalance() != DEFAULT_BALANCE) {
