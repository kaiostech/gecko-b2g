...
function test(
  ...
  return (
    setEmulatorGsmLocationAndWait(aLac, aCid)
      .then(() =>
      ...
      .then(() =>
        check(
          aExpectedLongName == null ? aLongName : aExpectedLongName,
...
startTestCommon(function() {
  ...
  return getEmulatorOperatorNames().then(function(aOperators) {
    ...
    let {
      gsmLocationAreaCode: lac,
      gsmCellId: cid,
    } = mobileConnection.voice.cell;
    ...
    return (
      Promise.resolve()
        ...
        .then(() =>
          test("Foo1", "Bar1", "001", "02", 0x0000, TEST_CELL_ID, "Android", "")
        )
        .then(() =>
          test("Foo2", "Bar2", "001", "02", 0x0001, TEST_CELL_ID, "Test2", "")
        )
        .then(() =>
          test("Foo3", "Bar3", "001", "02", 0x0002, TEST_CELL_ID, "Test2", "")
        )
        .then(() =>
          test("Foo4", "Bar4", "001", "02", 0x0010, TEST_CELL_ID, "Test2", "")
        )
        .then(() =>
          test("Foo5", "Bar5", "001", "02", 0xfffe, TEST_CELL_ID, "Android", "")
        )

        // Single entry network.  Report network name from PNN if lac matches.
        .then(() =>
          test("Foo1", "Bar1", "001", "03", 0x0000, TEST_CELL_ID, "Android", "")
        )
        .then(() =>
          test("Foo2", "Bar2", "001", "03", 0x0011, TEST_CELL_ID, "Test3", "")
        )
        .then(() =>
          test("Foo3", "Bar3", "001", "03", 0xfffe, TEST_CELL_ID, "Android", "")
        )

        // Test if we match MNC "01" and "001" correctly.
        .then(() =>
          test("Foo1", "Bar1", "001", "001", 0x0012, TEST_CELL_ID, "Test4", "")
        )

        // Wild char test for MCC = 001, MNC = 1D cases.
        .then(() =>
          test("Foo10", "Bar10", "001", "10", 0x0000, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo11", "Bar11", "001", "11", 0x0001, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo12", "Bar12", "001", "12", 0x0002, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo13", "Bar13", "001", "13", 0x0003, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo14", "Bar14", "001", "14", 0x0004, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo15", "Bar15", "001", "15", 0x0005, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo16", "Bar16", "001", "16", 0x0006, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo17", "Bar17", "001", "17", 0x0007, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo18", "Bar18", "001", "18", 0x0008, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test("Foo19", "Bar19", "001", "19", 0x0009, TEST_CELL_ID, "Test5", "")
        )
        .then(() =>
          test(
            "Foo20",
        ...
        .then(() =>
          test("Foo0", "Bar0", "001", "200", 0x00c8, TEST_CELL_ID, "Test6", "")
        )
        .then(() =>
          test("Foo1", "Bar1", "001", "299", 0x012b, TEST_CELL_ID, "Test6", "")
        )
        ...
        .then(() =>
          test(longName, shortName, mcc, mnc, lac, cid, "Android", "")
        )
    );
  });
});
