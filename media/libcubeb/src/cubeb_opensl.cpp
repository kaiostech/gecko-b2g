...
#define SL_BOOLEAN_CAST(condition)                                             \
  ((condition) ? SL_BOOLEAN_TRUE : SL_BOOLEAN_FALSE)
...
#endif
#if defined(B2G_VOICE_PROCESSING)
  ctx->SL_IID_ANDROIDACOUSTICECHOCANCELLATION = *(SLInterfaceID *)dlsym(
      ctx->lib, "SL_IID_ANDROIDACOUSTICECHOCANCELLATION");
  ctx->SL_IID_ANDROIDAUTOMATICGAINCONTROL =
      *(SLInterfaceID *)dlsym(ctx->lib, "SL_IID_ANDROIDAUTOMATICGAINCONTROL");
  ctx->SL_IID_ANDROIDNOISESUPPRESSION =
      *(SLInterfaceID *)dlsym(ctx->lib, "SL_IID_ANDROIDNOISESUPPRESSION");
#endif
...
opensl_configure_capture(cubeb_stream * stm, cubeb_stream_params * params)
{
...
#if defined(B2G_VOICE_PROCESSING)
        // If platform voice processing is enabled, try to request interfaces of
        // AEC/AGC/NS.
  ...
  if (res == SL_RESULT_SUCCESS) {
    res = (*stm->acousticEchoCancellationItf)
              ->SetEnabled(stm->acousticEchoCancellationItf,
                           SL_BOOLEAN_CAST(stm->enable_aec));
    if (res != SL_RESULT_SUCCESS) {
      LOG("Failed to %s AEC. Error code: %lu",
          stm->enable_aec ? "enable" : "disable", res);
    }
  } else {
  ...
  if (res == SL_RESULT_SUCCESS) {
    res = (*stm->automaticGainControlItf)
              ->SetEnabled(stm->automaticGainControlItf,
                           SL_BOOLEAN_CAST(stm->enable_agc));
    if (res != SL_RESULT_SUCCESS) {
      LOG("Failed to %s AGC. Error code: %lu",
          stm->enable_agc ? "enable" : "disable", res);
    }
  } else {
  ...
  if (res == SL_RESULT_SUCCESS) {
    res = (*stm->noiseSuppressionItf)
              ->SetEnabled(stm->noiseSuppressionItf,
                           SL_BOOLEAN_CAST(stm->enable_ns));
    if (res != SL_RESULT_SUCCESS) {
      LOG("Failed to %s NS. Error code: %lu",
          stm->enable_ns ? "enable" : "disable", res);
    }
  } else {
...
#if defined(B2G_VOICE_PROCESSING)
  stm->enable_aec =
      has_pref_set(input_stream_params, nullptr, CUBEB_STREAM_PREF_AEC);
  stm->enable_agc =
      has_pref_set(input_stream_params, nullptr, CUBEB_STREAM_PREF_AGC);
  stm->enable_ns =
      has_pref_set(input_stream_params, nullptr, CUBEB_STREAM_PREF_NS);
#endif
