...
var LEGACY = {
  saveRecord: function(aMessageRecord, aAddresses, aCallback) {
    ...
    this.newTxn(
      ...
      function(error, txn, stores) {
        ...
        LEGACY.replaceShortMessageOnSave.call(
          self,
          txn,
          messageStore,
          participantStore,
  ...
  replaceShortMessageOnSave: function(
    aTransaction,
    aMessageStore,
    aParticipantStore,
    aThreadStore,
  ...
  realSaveRecord: function(
    ...
    let self = this;
    this.findThreadRecordByPlmnAddresses(
      aThreadStore,
      aParticipantStore,
      aAddresses,
      true,
  ...
  saveReceivedMessage: function(aMessage, aCallback) {
    ...
    if (aMessage.type == "mms") {
      ...
      LEGACY.fillReceivedMmsThreadParticipants.call(
        this,
        aMessage,
        threadParticipants
      );
...
function generateMms(aSender, aReceivers, aDelivery) {
  ...
  for (let i = 0; i < aReceivers.length; i++) {
    ...
    ok(
      true,
      "MMS to address '" + receiver + "' resolved as type " + entry.type
    );
...
function populateDatabase(aMmdb) {
  ...
  let promise = Promise.resolve()
    ...
    .then(() =>
      saveReceivedMessageLegacy(
        aMmdb,
        generateSms("+15525225554", null, DELIVERY_RECEIVED)
      )
    )
    ...
    .then(() =>
      saveReceivedMessageLegacy(
        aMmdb,
        generateSms("5525225554", null, DELIVERY_RECEIVED)
      )
    );
  ...
  for (let i = 0; i < TEST_ADDRESSES.length; i++) {
    ...
    promise = promise.then(() =>
      saveReceivedMessageLegacy(
        aMmdb,
        generateMms(address, ["a"], DELIVERY_RECEIVED)
      )
    );
  }
  ...
  for (let i = 0; i < TEST_ADDRESSES.length; i++) {
    for (let j = i + 1; j < TEST_ADDRESSES.length; j++) {
      let addr_i = TEST_ADDRESSES[i],
        addr_j = TEST_ADDRESSES[j];
      promise = promise.then(() =>
        saveSendingMessageLegacy(
          aMmdb,
          generateMms(null, [addr_i, addr_j], DELIVERY_SENDING)
        )
      );
    }
  }
...
function doVerifyDatabase(aMmdb, aExpected) {
  ...
  return createThreadCursor(aMmdb).then(function(aValues) {
    ...
    return createMessageCursor(aMmdb).then(function(aValues) {
