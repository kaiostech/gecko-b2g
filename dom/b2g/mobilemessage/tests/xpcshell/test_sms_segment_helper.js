...
add_test(function test_SmsSegmentHelper_calculateUserDataLength() {
  function test_calc(str, expected, enabledGsmTableTuples, strict7BitEncoding) {
    ...
    let options = gSmsSegmentHelper.calculateUserDataLength(
      str,
      strict7BitEncoding
    );
  ...
  test_calc(
    "\u00e7",
    [PDU_DCS_MSG_CODING_7BITS_ALPHABET, 1, 3, 1, 0],
    [
      [1, 0],
      [2, 0],
    ]
  ...
  test_calc(
    "\u09fa",
    [PDU_DCS_MSG_CODING_7BITS_ALPHABET, 2, 6, 2, 4],
    [
      [2, 0],
      [2, 4],
    ]
  ...
  test_calc(
    "\\\\\\\\\\\\\\",
    [PDU_DCS_MSG_CODING_7BITS_ALPHABET, 14, 0, 0, 0],
    [
      [3, 1],
      [0, 0],
    ]
  ...
  test_calc(
    ...
    [PDU_DCS_MSG_CODING_7BITS_ALPHABET, 2, 0, 0, 0],
    [
      [3, 0],
      [0, 0],
    ]
  ...
  test_calc(
    str,
    [PDU_DCS_MSG_CODING_7BITS_ALPHABET, gsmLen, 0, 0, 0],
    [[0, 0]],
    true
  );
  test_calc(
    str,
    [PDU_DCS_MSG_CODING_16BITS_ALPHABET, str.length * 2, udhl],
    [[0, 0]]
  );
 ...
 */
add_test(
  function test_SmsSegmentHelper_calculateUserDataLength7Bit_multipart() {
...
add_test(function test_SmsSegmentHelper_fragmentText7Bit() {
  ...
  test_calc(
    generateStringOfLength("{", PDU_MAX_USER_DATA_7BIT / 2 + 1),
    false,
    2
  );
  // Escaped character cannot be separated
  test_calc(
    generateStringOfLength("{", ((PDU_MAX_USER_DATA_7BIT - 7) * 2) / 2),
    false,
    3
  );
  ...
  test_calc(
    generateStringOfLength("A", (PDU_MAX_USER_DATA_7BIT - 7) * 2),
    false,
    2
  );
  test_calc(
    generateStringOfLength("A", (PDU_MAX_USER_DATA_7BIT - 7) * 3),
    false,
    3
  );
  ...
  test_calc(
    generateStringOfLength("\ua2db", PDU_MAX_USER_DATA_UCS2 + 1),
    false,
    2
  );
  ...
  test_calc(
    generateStringOfLength(
      "\ua2db",
      ((PDU_MAX_USER_DATA_UCS2 * 2 - 7) * 2) / 2
    ),
    false,
    3
  );
