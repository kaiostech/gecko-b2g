...
class GonkBufferQueueCore : public virtual RefBase {
 ...
 private:
  ...
  // mIsAllocating indicates whether a producer is currently trying to allocate
  // buffers (which releases mMutex while doing the allocation proper).
  // Producers should not modify any of the FREE slots while this is true.
  // mIsAllocatingCondition is signaled when this value changes to false.
  bool mIsAllocating;
